...
- **SHA-256 hash** computed and stored (`integrity_hash` field) at the moment each time record
  is created. `GET /api/reports/integrity/{record_id}` recomputes the hash from the record's
  current fields and compares it to the stored one, returning an explicit `status`:
  - `verified`: the record has not been altered since creation.
  - `tampered`: a hashed field (`worker_id`, `company_id`, `type`, `timestamp`,
    `duration_minutes`, `created_at`) was changed directly in the database outside the audited
    correction workflow.
  - `legacy`: the record predates this capability and has no stored `integrity_hash` (does not
    imply tampering). Can be populated with `python -m api.manage_integrity backfill`.
  - Corrections approved via change-request automatically recompute the hash, so a legitimate,
    audited change is not flagged as tampering.
  - **Important**: this is tamper-**evidence** for parties without database access (auditors,
    workers, the managed cloud), not a cryptographic guarantee against whoever administers the
    server/database itself (self-hosted deployments).
